...
```{r plotSummary, fig.height= 15, fig.cap = "*Overall summary of FastQC reports*"}
plotSummary(rawFqc) +
...
```{r plotReadTotals, fig.height=15, fig.cap = "*Library Sizes for all supplied fastq files. Any samples run as multiple libraries are  shown as the supplied multiple libraries and have not been merged.*"}
...
```{r plotBaseQuals, fig.height=15, fig.cap = "*Mean sequencing quality scores at each base position for each library*"}
plotBaseQuals(
  rawFqc,
  pattern = suffix, 
  dendrogram = TRUE,
...
```{r plotGC, fig.height= 12, fig.cap=glue("*GC content shown as the % above and below the theoretical GC content for the {sp} transcriptome.*")}
plotGcContent(
  x = rawFqc, 
  pattern = suffix, 
  species = sp, 
...
```{r plotSeqContentResid, fig.width = 14, fig.cap = "*Base and Position specific residuals for each sample. The mean base content at each position was calculated for each nucleotide, and the sample-specific residuals calculated.*"}
plotly::ggplotly(
...
```{r plotAdapterContent, fig.height= 15, fig.cap="*Total Adapter Content for each sample shown by starting position in the read.*"}
plotAdapterContent(
  x = rawFqc, 
  pattern = suffix, 
  dendrogram = TRUE,
...
```{r plotOverRep, fig.height= 12, fig.cap = "*Summary of over-represented sequences across all libraries*"}
if (nrow(os)){
