...
tumor_sample %>%
    mutate(
        Diagnosis = ifelse(
            Diagnosis == "ILC-like", 
            "invasive carcinoma of no special type",
            "infiltrating carcinoma of no special type"
        )
    ) %>%
    ungroup() %>%
    dplyr::select(
        Patient = patient, Age, `Pathological Label` = Diagnosis
    ) %>%
    unique() %>% xtable::xtable(
        caption = "Age and pathological label of the 8 donors who we received the primary tumours from.",
        label = "pat_info"
    )  %>%
    xtable::print.xtable(
        "~/PhD_thesis/draft_figure/chapter_04/pat_info.tex", type = "latex",
