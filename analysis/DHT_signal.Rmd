...
Out of the `r nrow(AR_sig)` previously detected AR signatures, `r length(levels(ar_sig_overlap$gene_name))` of them were also implicated in pathways significantly perturbed by DHT in ductal-like tumours and had directional concordant changes. 
...
```{r}
ductal_2treat %>%
    dplyr::filter(group == "DHT & E2+DHT") %>%
    mutate(gs_name = str_remove_all(gs_name, "kegg.")) %>%
    ggplot(
        aes(Comparison, gs_name, fill = `t statistic`)
    ) +
    geom_tile() +
